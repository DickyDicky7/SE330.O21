...
public final class GameBoard extends Table {
    ...
    TextureRegion textureRegion(final Board board, final int tileID) {
        return board.getTile(tileID).isTileOccupied() ? GuiUtils.GetPieceTextureRegion(board.getTile(tileID).getPiece()) : GuiUtils.TRANSPARENT_TEXTURE_REGION;
    }

    public static final class DisplayOnlyBoard extends Table {

        private GuiUtils.TileColor tileColor;

        public DisplayOnlyBoard() {
            this.setFillParent(true);
            this.tileColor = GuiUtils.TileColor.CLASSIC;
            for (int i = 0; i < BoardUtils.NUM_TILES; i += 1) {
                if (i % 8 == 0) {
                    this.row();
                }
        ...
        private static Color getTileColor(final GuiUtils.TileColor TileColor, final int i) {
            if (BoardUtils.FIRST_ROW.get(i) || BoardUtils.THIRD_ROW.get(i) || BoardUtils.FIFTH_ROW.get(i) || BoardUtils.SEVENTH_ROW.get(i)) {
                return i % 2 == 0 ? TileColor.getLightTile() : TileColor.getUnlitTile();
            }
                return i % 2 != 0 ? TileColor.getLightTile() : TileColor.getUnlitTile();
        }

        private static Color getHighlightTileColor(final GuiUtils.TileColor TileColor, final int i) {
            if (BoardUtils.FIRST_ROW.get(i) || BoardUtils.THIRD_ROW.get(i) || BoardUtils.FIFTH_ROW.get(i) || BoardUtils.SEVENTH_ROW.get(i)) {
                return i % 2 == 0 ? TileColor.getHighlightLegalMoveLightTile() : TileColor.getHighlightLegalMoveUnlitTile();
            }
                return i % 2 != 0 ? TileColor.getHighlightLegalMoveLightTile() : TileColor.getHighlightLegalMoveUnlitTile();
        }

        public GuiUtils.TileColor getTileColor() {
            return this.tileColor;
        }

        public void setTileColor(final GuiUtils.TileColor tileColor) {
