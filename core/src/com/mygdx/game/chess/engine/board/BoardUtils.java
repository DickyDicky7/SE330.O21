...
public final class BoardUtils {
    ...
    public static final ImmutableMap<String, Integer> POSITION_TO_COORDINATE =
            initializePositionToCoordinateMap();
    public static final int
            DEFAULT_TIMER_MINUTE = 5,
            DEFAULT_TIMER_SECOND = 0,
            DEFAULT_TIMER_MILLISECOND = 0;
    ...
    public static boolean kingThreat(final Move move) {
        return move
                .getBoard()                  .currentPlayer()
                .makeMove(move).latestBoard().currentPlayer().isInCheck();
    }
    ...
    private static ImmutableList<Integer> generateColumnList(
            final  ImmutableList<Integer> columnList, final int column) {
        if (column < NUM_TILES) {
            return generateColumnList (ImmutableList.<Integer>builder().addAll(columnList)
                    .add(column).build(), column + NUM_TILES_PER_ROW);
        } else {
            return columnList;
        }
    }
    ...
    private static ImmutableList<Boolean> initColumn(final int column) {
        ...
        return ImmutableList.copyOf(getBoardNumStream()
                .map(columnList::contains).collect(Collectors.toList()));
    }

    private static ImmutableList<Integer> generateRowList(
            final  ImmutableList<Integer> rowList, final int row, final boolean firstColumn) {
        if (firstColumn || row % NUM_TILES_PER_ROW != 0) {
            return generateRowList(ImmutableList.<Integer>builder()
                    .addAll(rowList).add(row).build(), row + 1, false);
        } else {
            return rowList;
        }
    }
    ...
    private static ImmutableList<Boolean> initRow(int row) {
        final   ImmutableList<Integer> rowList = generateRowList(
                ImmutableList.of(), row, true);
        return  ImmutableList.copyOf(getBoardNumStream().map(rowList::contains).collect(Collectors
                .toList()));
    }
    ...
    public static int mostValuableVictimLeastValuableAggressor(final Move move) {
        ...
        if (move.isAttack()) {
            return (move.getAttackedPiece().getPieceValue() - movingPieceValue
                    + PieceType.KING.getPieceValue()) * 100;
        } else {
            return    PieceType.KING.getPieceValue()        - movingPieceValue;
        }
    }
    ...
    private static ImmutableMap<String, Integer> initializePositionToCoordinateMap() {
        return Maps.uniqueIndex(getBoardNumStream().collect(Collectors.toList(
        )), ALGEBRAIC_NOTATION::get);
    }

    public static Piece getPieceAtPosition(final Board board, final String position) {
        return board.getAllPieces().parallelStream()
                .filter(piece -> piece.getPiecePosition(
                ) == BoardUtils.getCoordinateAtPosition(position))
                .findFirst().orElseThrow(() -> new IllegalStateException("Invalid Piece"));
    }
}
