...
public abstract class Player {
    ...
    private King establishKing() {
        return ((King) this.getActivePieces().parallelStream().filter(piece -> piece.getPieceType() == PieceType.KING).findFirst().orElseThrow(() -> new IllegalStateException("Invalid board")));
    }
    ...
    protected final boolean noEscapeMoves() {
        return this.legalMoves.parallelStream().noneMatch(move -> this.makeMove(move).moveStatus().isFinished());
    }
    ...
    public final MoveTransition makeMove(final Move move) {
        ...
        if (transitionBoard != null) {
            ...
            if (!kingAttacks.isEmpty()) {
                return new MoveTransition(board, board, MoveStatus.LEAVING_PLAYER_IN_CHECK);
            }
