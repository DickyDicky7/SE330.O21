...
import com.mygdx.game.chess.engine.board.MoveLogger;
...
public final class FenUtilities {
    ...
    public static String getGameData(final MoveLogger moveLogger, final Board board) {
        final ImmutableList<Coordinate> coordinateList = ImmutableList.copyOf(moveLogger.getMoves().parallelStream().map(move -> new Coordinate(move.getCurrentCoordinate(), move.getDestinationCoordinate())).collect(Collectors.toList()));
        return formGameData(coordinateList, "", 0).trim() + getPlayerTimer(board);
    }

    public static Board createGameFromSavedData(final String data, final MoveLogger moveLogger) {
        ...
        final MoveLogger updatedMoveLogger = new MoveLogger();
        for (int i = 0; i < coordinates.length; i++) {
            ...
            updatedMoveLogger.addMove(move);
            board = board.currentPlayer().makeMove(move).latestBoard();
        }
        moveLogger.addAllMove(updatedMoveLogger.getMoves());
        return board;
    }
    ...
    private record Coordinate(int currentCoordinate, int destinationCoordinate) {
    }
}
